...
describe('普通笔簇合并（persistContentStroke 一笔一 mark·事后聚成一条 KO，减少导出碎片）', () => {
  function contentMark(over: Partial<PersistedMark> = {}): PersistedMark {
    return mark({
      ...
      marked_text: '',
      hmp: null,
      strokes: [{ tool: 'pen', points: [{ x: 0.1, y: 0.1, t: 0, pressure: 0.5 }], coord_space: 'page_norm', capture_surface: 'page' }],
      ...over,
    });
  }

  it('同页+短间隔+空间相邻的连续内容笔合成一条 KO：provenance.mark_ids 多笔、anchor_bbox 是 union、ko_id 沿用首笔身份', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const m2 = contentMark({ mark_id: 'c2', seq: 2, abs_timestamp: 1400, bbox: [0.12, 0.10, 0.02, 0.02] }); // 400ms 后·挨着
    const m3 = contentMark({ mark_id: 'c3', seq: 3, abs_timestamp: 1900, bbox: [0.14, 0.11, 0.02, 0.02] }); // 再 500ms·挨着
    const kos = await assembleKnowledgeObjects(input([m1, m2, m3], []));

    expect(kos).toHaveLength(1); // 三笔碎片合并成一条 KO，不再刷屏三个文件
    const ko = kos[0];
    expect(ko.provenance).toEqual({ created_from: 'mark', mark_ids: ['c1', 'c2', 'c3'] });
    expect(ko.source.anchor_bbox).toEqual([0.10, 0.10, 0.06, 0.03]); // union of 三笔 bbox
    expect(ko.body_md).toBe(INK_PLACEHOLDER_DRAWING);

    // 单独用 assembleKnowledgeProjection 核对 ko_id 身份沿用首笔（stableKey 用 group[0]）
    const soloKo = await assembleKnowledgeObjects(input([m1], []));
    expect(ko.ko_id).toBe(soloKo[0].ko_id);
  });

  it('间隔超过阈值（>2.5s）不合并：各自独立成 KO', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const m2 = contentMark({ mark_id: 'c2', seq: 2, abs_timestamp: 1000 + 5000, bbox: [0.11, 0.10, 0.02, 0.02] }); // 5s 后·挨着但太久
    const kos = await assembleKnowledgeObjects(input([m1, m2], []));
    expect(kos).toHaveLength(2);
    expect(kos.map((k) => k.provenance.mark_ids).sort()).toEqual([['c1'], ['c2']]);
  });

  it('空间上离得远（超出邻近半径）不合并：各自独立成 KO', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const m2 = contentMark({ mark_id: 'c2', seq: 2, abs_timestamp: 1200, bbox: [0.80, 0.80, 0.02, 0.02] }); // 时间近但离得很远
    const kos = await assembleKnowledgeObjects(input([m1, m2], []));
    expect(kos).toHaveLength(2);
  });

  it('已识别出文字的内容笔不参与合并（是有意义的独立单元）', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const m2 = contentMark({ mark_id: 'c2', seq: 2, abs_timestamp: 1400, bbox: [0.12, 0.10, 0.02, 0.02], hmp: hmp({ text_hint: '识别出的文字' }) });
    const kos = await assembleKnowledgeObjects(input([m1, m2], []));
    expect(kos).toHaveLength(2); // m2 已识别出文字·不与 m1 合并
    expect(kos.find((k) => k.body_md === '识别出的文字')).toBeTruthy();
  });

  it('带 entity_refs 的内容笔不参与合并（用户显式声明的关系不该被邻居粘走）', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const m2 = contentMark({ mark_id: 'c2', seq: 2, abs_timestamp: 1400, bbox: [0.12, 0.10, 0.02, 0.02], entity_refs: [ref('some-entity')] });
    const kos = await assembleKnowledgeObjects(input([m1, m2], []));
    expect(kos).toHaveLength(2);
  });

  it('高亮（markup/excerpt）不参与合并：多段高亮各自独立，不混淆标了哪一段', async () => {
    const m1 = mark({ mark_id: 'e1', feature_type: 'markup', marked_text: '第一段', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.4, 0.02] });
    const m2 = mark({ mark_id: 'e2', feature_type: 'markup', marked_text: '第二段', seq: 2, abs_timestamp: 1400, bbox: [0.10, 0.13, 0.4, 0.02] });
    const kos = await assembleKnowledgeObjects(input([m1, m2], []));
    expect(kos).toHaveLength(2);
  });

  it('已被 ai_turn 消费的 mark 不参与聚簇（打断簇的连续性，与既有消费语义一致）', async () => {
    const m1 = contentMark({ mark_id: 'c1', seq: 1, abs_timestamp: 1000, bbox: [0.10, 0.10, 0.02, 0.02] });
    const consumedMark = mark({ mark_id: 'x1', feature_type: 'markup', marked_text: 'anchor', seq: 2, abs_timestamp: 1200 });
    const m3 = contentMark({ mark_id: 'c3', seq: 3, abs_timestamp: 1400, bbox: [0.12, 0.10, 0.02, 0.02] });
    const t = aiTurn({ overlay_id: 'ov1', ai_reply: 'reply', anchor: { surface_id: 's', mark_ids: ['x1'], object_refs: [] } });
    const kos = await assembleKnowledgeObjects(input([m1, consumedMark, m3], [t]));
    // ai_note(x1) + c1、c3 各自独立（consumed 打断了 c1/c3 的连续性，不该跨过被消费的 mark 强行粘一起）
    expect(kos).toHaveLength(3);
    expect(kos.filter((k) => k.kind === 'annotation')).toHaveLength(2);
  });
});
