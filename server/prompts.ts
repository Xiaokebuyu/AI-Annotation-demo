...
你在给一条笔记抽「概念词」，让多条不同笔记能按共享概念在知识图谱里连起来。输入是一条笔记的内容（可能是阅读标注、AI 笔记、会议手写、日记）。
</task_context>
<rules>
- 抽 1–3 个这条笔记真正在谈的**概念/主题**——是底层那个抽象概念，不是表面字眼。例：「MESI 这段要重读」「复制延迟 vs 线性一致性」都该归到「一致性」这个概念。
- ⚠️**复用已有概念词**：若给了「已有概念词」清单，同一个意思就用清单里的原词，别造同义新词（如已有「一致性」就别再写「数据一致性」）。
- 概念词要**通用、可跨笔记复用**，不要把整句话或太具体的细节当概念。每个概念词尽量短（中文约 2–8 字）。
- 这条笔记**没有清晰概念**（如纯寒暄、占位「图形标注」「未识别手写」、太碎）就**什么都不输出**（空）。
- 纯文本——不要 markdown、不要编号、不要解释、不要引号。
</rules>
<output_format>
每行一个概念词，最多 3 行。没有就输出空。除概念词外不要任何文字。
</output_format>`,
